...
const { KEY, MI_TOKEN } = process.env;
...
exports.getWorkspaceId = async (req, res) => {
  ...
  try {
    const response = await axios.get(
      `https://api.trello.com/1/members/me/organizations?key=${KEY}&token=${MI_TOKEN}`
    );
...
exports.createBoard = async (req, res) => {
  ...
  try {
    ...
    const workSpace = worksSpaces.find(
      (org) => org.displayName === nameWorkspace
    );
    const params = {
      name,
      desc,
...
exports.getBoardId = async (req, res) => {
  ...
  try {
    const response = await axios.get(
      `https://api.trello.com/1/members/me/boards?key=${KEY}&token=${MI_TOKEN}`
    );
    console.log(response.data);
...
exports.createList = async (req, res) => {
  ...
  try {
    ...
    const response = await axios.post(
      `https://api.trello.com/1/lists?key=${KEY}&token=${MI_TOKEN}`,
      listData
    );
...
exports.createCard = async (req, res) => {
  const { cardName, listName, boardName } = req.body; // Nombre de la tarjeta, nombre de la lista y ID del tablero desde el cuerpo de la solicitud
  ...
  try {
    ...
    const listsResponse = await axios.get(
      `https://api.trello.com/1/boards/${board.id}/lists?key=${KEY}&token=${MI_TOKEN}`
    );
    ...
    if (listsResponse.status === 200) {
      ...
      if (targetList) {
        ...
        const cardResponse = await axios.post(
          `https://api.trello.com/1/cards?key=${KEY}&token=${MI_TOKEN}`,
          cardData
        );
